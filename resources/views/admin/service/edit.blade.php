...
<body>
    ...
    <h2 style="text-align: center;">Edit services</h2>
    <div style="text-align: center; margin-bottom: 20px; padding-top:20px;">
        <a href="/services" style="background-color: #4CAF50; color: #ffffff; padding: 10px 20px; border: none; border-radius: 5px; cursor: pointer;">Kembali ke services</a>
    </div>
    <form action="{{ route('services.update', $services->id) }}" method="POST" enctype="multipart/form-data" style="max-width: 500px; margin: 40px auto; padding: 20px; background-color: #f9f9f9; border: 1px solid #ddd; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1);">
        @csrf

        <div class="form-group mb-4">
            <label for="" style="display: block; margin-bottom: 10px; font-weight: bold; color: #333;">Masukan Judul</label>
            <input type="text" class="form-control @error('judul') is-invalid @enderror" name="judul" value="{{ old('judul', $services->judul) }}" style="width: 100%; height: 40px; padding: 10px; font-size: 16px; border: 1px solid #ccc; border-radius: 5px;">
            @error('judul')
        ...
        <div class="form-group mb-4">
            <label for="" style="display: block; margin-bottom: 10px; font-weight: bold; color: #333;">Masukan subJudul</label>
